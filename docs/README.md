# Documentation

This directory contains the complete documentation for the Metrics Billing Platform.

## Structure

- **VitePress Site** - User-facing documentation website (run `npm run docs:dev`)
- **Reference Docs** - Detailed markdown files for developers

## Running the Documentation Site

```bash
# Install dependencies
cd docs
npm install

# Run development server
npm run dev

# Build for production
npm run build
```

Or from project root:

```bash
npm run docs:dev
```

## Documentation Organization

- `getting-started/` - Installation, configuration, first steps, deployment
- `api/` - Complete API reference
- `architecture/` - System architecture and design
- `operations/` - Operations guides and troubleshooting
- `status/` - Project status and implementation details
- `*.md` - Reference documentation files

## See Also

- [Main README](../README.md)
- [Documentation Index](/INDEX)
